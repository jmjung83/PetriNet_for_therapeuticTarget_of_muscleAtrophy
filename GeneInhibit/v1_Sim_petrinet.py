...
def petriNet_simulator(place_dic,
    ...
    result_place_file = open(cur_path + "/result_place_%s%s.txt"%(mode, ITER), 'w+')
    ...
    check_target_exist = place_dic[INH_PLC]
    # a gene inhibition
    if INH_PLC in rt_phos_inhi_set:
        place_dic[INH_PLC] = UP_effect
    else:
        place_dic[INH_PLC] = 0
    ...
    result_transition_file = open(cur_path + "/result_transition_%s%s.txt"%(mode, ITER),'w+')
